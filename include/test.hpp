 ...
 *
 * This class contains a feedback loop control method to move the robot to a
 * goal position.
 *
 */
class BotController : public rclcpp::Node {
 public:
  BotController(std::string node_name, std::string robot_name,
                bool go_to_goal = false, double linear_speed = 0.3,
  ...
  {
    auto current_location = std::make_pair<double, double>(0.0, 0.0);
    ...
    // this->create_subscription<nav_msgs::msg::Odometry>(pose_topic_name, 10,
    // std::bind(&BotController::robot_pose_callback, this,
    // std::placeholders::_1));
    ...
    m_transform_timer = this->create_wall_timer(
        ...
        std::bind(&BotController::transform_callback, this), m_cbg);
    m_go_to_goal_timer = this->create_wall_timer(
        std::chrono::milliseconds((int)(1000.0 / 1)),
        std::bind(&BotController::go_to_goal_callback, this), m_cbg);
  }
